  ...
  const techCategories = {
    ...
    product: {
      ...
      technologies: technologies.filter(tech =>
        ['Figma', 'Jira', 'Confluence', 'Notion'].includes(tech.name)
      )
    },
    analytics: {
      name: 'Analytics & Data',
      icon: '📊',
      technologies: technologies.filter(tech =>
        ['Amplitude', 'Metabase', 'Databricks', 'New Relic', 'Grafana'].includes(tech.name)
      )
    },
    technical: {
      name: 'Technical',
      icon: '💻',
      technologies: technologies.filter(tech =>
        ['SQL', 'Python', 'JavaScript', 'Postman', 'GitHub', 'VS Code'].includes(tech.name)
      )
    }
  };
      ...
      <div className="relative max-w-7xl mx-auto px-4 mt-0">
        ...
        >
          ...
          <motion.p 
            ...
            transition={{ duration: 0.3 }}
          >
            A comprehensive toolkit I use to build and scale products — spanning product
            design, analytics, and the technical skills that help me work closely with
            engineering teams.
          </motion.p>
        </motion.div>
