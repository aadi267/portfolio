...
const OceanicHeroText = () => (
  <div className="flex-1 max-w-5xl text-center px-4 sm:px-0">
    ...
    >
      {['Product Strategy', 'Fintech', 'Growth', 'A/B Testing', 'User Research', 'Analytics'].map((tech, index) => (
        <span
          key={tech}
...
const OceanicStatsOverlay = () => {
  const stats = [
    { label: 'Experience', value: '4+ Years', icon: '🚀', color: 'sky' },
    { label: 'GTV Driven', value: '₹4500Cr+', icon: '📈', color: 'teal' },
    { label: 'Users Impacted', value: '10M+', icon: '👥', color: 'blue' },
  ];
...
const OceanicCTAButtons = () => {
  const buttons = [
    {
      href: "mailto:adityaduveyy@gmail.com",
      text: "Email Me",
      icon: (
    ...
    {
      href: "mailto:adityaduveyy@gmail.com?subject=Request%20for%20a%20Google%20Meet&body=Hi%20Aditya%2C%0A%0AI%27d%20like%20to%20schedule%20a%20Google%20Meet%20with%20you.%20Please%20share%20a%20meeting%20link%20and%20a%20time%20that%20works.%0A%0AThanks%21",
      text: "Schedule Call",
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      >
        {buttons.map((button) => (
          <a
            ...
            {...(button.external && !button.download && { target: "_blank", rel: "noopener noreferrer" })}
